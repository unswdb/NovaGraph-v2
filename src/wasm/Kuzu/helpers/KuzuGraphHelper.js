...
export default class KuzuGraphHelper {
   ...
   * @returns {Object} Result of the query
   */
  createRelationship(fromNodeLabel, fromNodeProps, toNodeLabel, toNodeProps, relType, relProps = {}) {
    ...
    const relPropsString = relPropsArray.length > 0 ? ` {${relPropsArray.join(', ')}}` : '';
  ...
  findNodesByRelationships(nodeLabel, relationshipTypes, limit = 100) {
    const relTypePattern = relationshipTypes.map(type => `:${type}`).join('|');
   ...
   */
  listRelationshipProperties(fromNodeLabel, fromNodeProps, toNodeLabel, toNodeProps, relType) {
    const fromConditions = this._formatPropertyConditions(fromNodeProps, 'a');
    const toConditions = this._formatPropertyConditions(toNodeProps, 'b');
  ...
  * @returns {Object} Result of the query
  */
  updateRelationshipProperties(fromNodeLabel, fromNodeProps, toNodeLabel, toNodeProps, relType, updateProps) {
    const fromConditions = this._formatPropertyConditions(fromNodeProps, 'a');
    const toConditions = this._formatPropertyConditions(toNodeProps, 'b');
  ...
  * @returns {Object} Result of the query
  */
  removeRelationship(fromNodeLabel, fromNodeProps, toNodeLabel, toNodeProps, relType) {
    const fromConditions = this._formatPropertyConditions(fromNodeProps, 'a');
    const toConditions = this._formatPropertyConditions(toNodeProps, 'b');
