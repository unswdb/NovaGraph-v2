...
bool process_json(const std::string &filename)
{
    ...
    if (!nodes.IsArray())
        throw std::runtime_error("Invalid nodes format in JSON file");
    if (!edges.IsArray())
        throw std::runtime_error("Invalid edges format in JSON file");
    if (!directed.IsBool())
        throw std::runtime_error("Invalid directed format in JSON file");
