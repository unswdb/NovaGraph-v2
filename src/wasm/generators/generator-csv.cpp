...
NodeMap process_nodes_csv(const std::string &filename)
{
    ...
    if (!std::getline(file, line))
        throw std::runtime_error("Could not read the Nodes CSV header");
    std::string header = trim(line);
    if (header != "nodes" && header != "Nodes")
        throw std::runtime_error("Incorrect header in nodes file");
...
void process_edges_csv(const std::string &edgesFilename, std::unordered_map<std::string, int> &nodeMap, bool directed)
{
    ...
    if (!file.is_open())
        throw std::runtime_error("Could not open file " + edgesFilename);
    ...
    while (std::getline(file, line))
    {
        std::vector<std::string> tokens = split(trim(line), ',');
        if (tokens.size() < 2)
            continue; // skip lines with less than 2 columns
        src = tokens[0];
        tar = tokens[1];
    ...
    {
        SETVAS(&globalGraph, "label", pair.second, pair.first.c_str());
    }

    // store weights in global variable
    igraph_vector_destroy(&globalWeights);
    if (weighted)
        igraph_vector_init_copy(&globalWeights, weights.vec());
}
