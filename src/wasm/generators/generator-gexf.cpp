...
val graph_from_gexf(const std::string &filename)
{
    ...
    if (!parsed)
        throw std::runtime_error("Error parsing GEXF file: " + std::string(parsed.description()));

    pugi::xml_node graph_element = doc.child("gexf").child("graph");
    if (!graph_element)
        throw std::runtime_error("No graph element found in GEXF file");
    ...
    for (pugi::xml_node node : graph_element.child("nodes").children("node"))
    {
        ...
        if (nodeMap.find(str_id) != nodeMap.end())
            throw std::runtime_error("Duplicate node id found: " + str_id);
        if (std::find(labels.begin(), labels.end(), label) != labels.end())
            throw std::runtime_error("Duplicate node label found: " + label);
    ...
    {
        SETVAS(&globalGraph, "label", i, labels[i].c_str());
    }
